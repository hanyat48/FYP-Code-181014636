...
function FSharp = CreateTData(FilePath)
    ...
    imshow(contrast);
    imwrite(contrast,'TrainingDataCreated/TDThresholdData.png');
    
    %uses Fast local laplacian filtering of images
    %to smooth the edges
    L = locallapfilt (I, 0.3, 4.0);
    %Saves the image
    imwrite(L,'TrainingDataCreated/TDtest10.png','PNG');
    %the usage of BM3D to deblur it
    [~, DNI] = BM3D(I,L,100,'np',0);
    %Saves the image
    imwrite(DNI,'TrainingDataCreated/TDtest11.png','PNG');
    %the usage of BM3DDEB to deblur it
    [~, DBI] = BM3DDEB(3,'TrainingDataCreated/TDtest11.png');
    %Saves the image
    imwrite(DBI,'TrainingDataCreated/TDtest12.png','PNG');
    ...
    %Saves the image
    imwrite(FSharp,'TrainingDataCreated/TDtest14.png','png');
...
%    disp(Sssim);   
%    %Thresholding
%    contrast= rgb2gray(contrast);
%    disp(size(contrast));disp(size(I));
